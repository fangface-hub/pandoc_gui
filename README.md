...
### Create Folder Distribution with Nuitka
...
2. Create executable:
    ...
    __Note__: `main_window.py` is included in the repository with post-build processing configured to place `filters/` and `locales/` outside `main_window.dist/`.
    ...
    Only if you need to regenerate the `Nuitka` file (normally not required):
    ...
    __Important__: You must manually add post-build processing to the `Nuitka` file generated by the above command.

3. Build output is in `dist/main_window.dist/`:
    ...
    The post-build processing in the `Nuitka` file places `filters/`, `locales/`, and `stylesheets/` outside `main_window.dist/`

### Bump Version Number

Use these scripts in the repository root:

- `./bump_patch.ps1`: `X.Y.Z` -> `X.Y.(Z+1)`
- `./bump_minor.ps1`: `X.Y.Z` -> `X.(Y+1).0`
- `./bump_major.ps1`: `X.Y.Z` -> `(X+1).0.0`

When you bump a higher version, lower versions are reset to zero.
All scripts update these files at the same time:
...
5. Select installer:
    ...
    - Click "Browse" and select `dist/main_window.dist/PandocGUI.exe`
