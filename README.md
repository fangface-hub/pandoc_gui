...
2. 実行ファイルを作成 / Create executable:

    **注意**: `PandocGUI.spec`はリポジトリに含まれており、ビルド後処理（`filters/`と`locales/`を`_internal/`の外に配置）が設定済みです。
    **Note**: `PandocGUI.spec` is included in the repository with post-build processing configured to place `filters/` and `locales/` outside `_internal/`.
    ...
    **重要**: 上記コマンドで生成した`.spec`ファイルには、手動でビルド後処理を追加する必要があります。
    **Important**: You must manually add post-build processing to the `.spec` file generated by the above command.

3. ビルド結果は`dist/PandocGUI/`に出力されます / Build output is in `dist/PandocGUI/`:

    ```text
    dist/PandocGUI/
    ├── PandocGUI.exe        # 実行ファイル / Executable
    ├── filters/             # Luaフィルター / Lua filters
    ├── locales/             # 翻訳ファイル / Translation files
    ├── profiles/            # プロファイル（実行時作成）/ Profiles (created at runtime)
    └── _internal/           # Python依存関係 / Python dependencies
    ```

    `.spec`ファイルのビルド後処理により、`filters/`と`locales/`が`_internal/`の外に配置されます。
    The post-build processing in the `.spec` file places `filters/` and `locales/` outside `_internal/`
